...
This document describes the architecture, patterns, and conventions used in
izi-queue. It is written for anyone working on the codebase, human or agent,
and is the file coding agents read first.

`AGENTS.md` is the canonical name. `CLAUDE.md` is a symlink to it, so tools
looking for either find the same document and there is only ever one copy to
keep current.
